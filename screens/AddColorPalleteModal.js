...
const AddColorPalleteModal = ({ navigation }) => {
  ...
  const Submit = useCallback(() => {
    if (title.length <= 0) {
      ...
      const addedColorPalette = {
        paletteName: title,
        colors: colors,
      };
      navigation.navigate("Home", { addedColorPalette });
    }
  }, [title, colors]);
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      <View style={styles.container}>
        <View>
          <Text style={styles.inputLabel}>Add Color Pallete Name:</Text>
          <TextInput
            style={styles.input}
            onChangeText={setTitle}
            value={title}
            placeholder="Color Pallete Name"
          />
        </View>
        <FlatList
          data={COLORS}
      ...
      <TouchableOpacity style={styles.submitWrap} onPress={Submit}>
...
const COLORS = [
  ...
  { colorName: "YellowGreen", hexCode: "#9ACD00" },
];
export default AddColorPalleteModal;
